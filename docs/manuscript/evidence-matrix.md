...
| The DepMap layer survives first-pass covariate control. | The upstream/downstream essentiality separation is not explained by expression breadth, tissue-specificity tau, gene length, or paralog count in the primary contrast. | `results/tables/interface_essentiality_regression_results.txt`; `results/reports/interface-essentiality-interpretation.md` | This is an OLS covariate screen on 54 primary genes, not a definitive causal model. |
